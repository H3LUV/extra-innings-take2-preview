(() => {
  ...
  const cache = new Map();
  const STORAGE_PREFIX = 'modetour_nearby_v2:';
  let timer;
  let runToken = 0;
  ...
  function readStored(key) {
    try {
      const item = JSON.parse(localStorage.getItem(STORAGE_PREFIX + key) || 'null');
  ...
  function writeStored(key, value, ttl) {
    try { localStorage.setItem(STORAGE_PREFIX + key, JSON.stringify({ value, expires: Date.now() + ttl })); }
    catch {}
  }
  ...
  async function geocode(city) {
    ...
    writeStored(key, result, 7 * 86400000);
    return result;
  }
  ...
  async function firstOverpass(query) {
    const endpoints = [
      'https://overpass.kumi.systems/api/interpreter',
      'https://overpass-api.de/api/interpreter',
      'https://overpass.nchc.org.tw/api/interpreter'
    ];
    return Promise.any(endpoints.map(endpoint => overpass(endpoint, query)));
  }

  function normalizeCities(data, geo) {
  ...
  async function loadNearbyCities(geo) {
    const key = `cities:${geo.latitude.toFixed(3)},${geo.longitude.toFixed(3)}`;
    if (cache.has(key)) return cache.get(key);
    const stored = readStored(key);
    if (stored?.length) { cache.set(key, stored); return stored; }

    for (const radius of [120000, 260000]) {
      try {
        const query = `[out:json][timeout:12];node(around:${radius},${geo.latitude},${geo.longitude})[place~"city|town"][name];out body 90;`;
        const cities = normalizeCities(await firstOverpass(query), geo);
        if (cities.length >= 3) {
          cache.set(key, cities);
          writeStored(key, cities, 7 * 86400000);
          return cities;
        }
      } catch {}
    }
    throw new Error('nearby-cities-empty');
  }
  ...
  async function loadWeather(city) {
    ...
    const result = { ...city, temp:Number(current.temperature_2m), feel:Number(current.apparent_temperature), code:Number(current.weather_code), wind:Number(current.wind_speed_10m), rain:Number.isFinite(rain) ? rain : null };
    writeStored(key, result, 10 * 60000);
    return result;
  }
  ...
  function renderLoading(city) {
    ...
    if (!grid) return;
    grid.className = 'nearGrid nearbyWeatherGrid';
    grid.innerHTML = Array.from({length:4}, () => '<article class="nearbyWeatherCard nearbyWeatherLoading"><div></div><h3>주변 도시 확인 중</h3><p>날씨를 불러오고 있습니다.</p></article>').join('');
    if ($('nearText')) $('nearText').textContent = `${city} 주변 주요 도시를 빠르게 찾고 있습니다.`;
  }
  ...
  async function update() {
    ...
    const grid = $('nearGrid');
    if (grid?.dataset.nearbyWeatherCity === city && grid.querySelector('.nearbyWeatherCard:not(.nearbyWeatherLoading)')) return;
    const token = ++runToken;
    renderLoading(city);
    try {
      const geo = await geocode(city);
      const nearby = await loadNearbyCities(geo);
      const weather = await Promise.allSettled(nearby.slice(0,4).map(loadWeather));
  ...
  document.addEventListener('submit', () => schedule(300), true);
  document.addEventListener('click', () => schedule(350), true);
  new MutationObserver(() => schedule(450)).observe(document.body, {childList:true, subtree:true, characterData:true});
