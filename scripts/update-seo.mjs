...
const SITE='https://today-mlb.pages.dev';
const NAME='오늘의 MLB';
...
for(const item of items){
  ...
  const tags=article.tags||['MLB','메이저리그','야구 분석'];
  const jsonLd={
  ...
  const body=(article.body||[]).map(p=>`<p>${esc(p)}</p>`).join('');
...
<body><header><div class="bar"><a href="../../#top">${NAME}</a><span>Created by H3LUV</span></div></header><main class="article-wrap"><article id="article"><div class="kicker">${esc(article.eyebrow||"TODAY'S COLUMN")}</div><h1 class="title">${esc(article.title)}</h1><p class="dek">${esc(article.dek||'')}</p><div class="meta">${BYLINE} · ${esc(published.slice(0,10))}</div>${article.thesis?`<div class="thesis">${esc(article.thesis)}</div>`:''}<div class="body">${body}</div>${takeaways}<div class="share"><a class="primary" href="../">지난 칼럼</a><a class="secondary" href="../../#top">오늘의 MLB 홈</a></div></article></main></body></html>`;
...
console.log(`SEO assets generated for ${items.length} MLB columns`);